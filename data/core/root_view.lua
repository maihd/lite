...
local function draw_intro(x, y, color, logo_color, command_color)
    ...
    command_color = command_color or color

    local th = style.big_font:get_height()
    local dh = th + style.padding.y * 2
    x = renderer.draw_text(style.big_font, "Lite", x, y + (dh - th) / 2, logo_color)
    x = x + style.padding.x
    ...
    for _, line in ipairs(lines) do
        local cmd_text = keymap.get_binding(line.cmd)

        local line_w = 0
        line_w = renderer.draw_text(style.font, cmd_text, x + style.padding.x, y, command_color)
        line_w = renderer.draw_text(style.font, line.text, x + style.padding.x + style.font:get_width(cmd_text), y, color)
